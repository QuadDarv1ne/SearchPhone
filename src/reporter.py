            ...
            with open(filename_csv, "w", encoding="utf-8", newline="") as f:
                ...
                for source in ["google", "twitter", "vk", "telegram", "reddit", "github", "duckduckgo"]:
                    for item in results.get(source, []):
                        writer.writerow(
                            [source.capitalize(), item.get("title", ""), item.get("link", ""), item.get("snippet", ""), item.get("created", "")]
                        )
    ...
    def export_pdf(self, phone_number, region, timestamp, results):
        ...
        try:
            ...
            if phone_info:
                ...
                tz_list = phone_info.get("timezone", ["N/A"])
                tz_str = ", ".join(str(tz) for tz in tz_list)
                pdf.cell(190, 6, f"  Timezones: {tz_str}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            ...
            for source in ["google", "duckduckgo_html", "yandex", "twitter", "vk", "telegram", "reddit", "github", "classifieds"]:
                if results.get(source):
                    if font_added:
                        ...
                        pdf.set_font("Helvetica", "B", 12)
                    pdf.cell(190, 8, source.replace("duckduckgo_html", "DUCKDUCKGO").upper(), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                    ...
                    for i, item in enumerate(results[source][:5], 1):
                        ...
                        link = item.get("link", "")
                        snippet = _clean_pdf_text(item.get("snippet", ""))
